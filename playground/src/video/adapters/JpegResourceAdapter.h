...
//			int  channels, color_type;
				...
				jcprops.JPGFile = filename.c_str();
				if((jerr = ijlRead(&jcprops, IJL_JFILE_READPARAMS)) != IJL_OK) {
					logger->error("Error al intentar leer el archivo [%s]", filename.c_str());
					ijlFree(&jcprops);
					return(null);
				};

				jcprops.JPGSizeBytes =  (jcprops.JPGWidth * 24 + 7) / 8 * jcprops.JPGHeight;
				logger->debug("Archivo [%s]:\n	Dimensiones: [%d]x[%d]\n	Tamaño: [%d]\n", filename.c_str(), jcprops.JPGWidth, jcprops.JPGHeight, jcprops.JPGSizeBytes);
				...
				resource->setBpp(24);
//				channels = jcprops.DIBChannels = 3;
