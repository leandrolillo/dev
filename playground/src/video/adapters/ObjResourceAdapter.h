    ...
    virtual Resource* load(FileParser &fileParser, const String &mimeType) {
        ...
        while ((token = fileParser.takeToken()) != FileParser::eof) {
            if (token == "o") {
                ...
                logger->info("skipping [%s] [%c] [%d]", line.c_str(), line.c_str()[0], line.c_str()[0]);
            }
        }
        ...
        for(unsigned int index = 0; index < indices.size(); index++) {
            ...
            if(previousSameVertexDiffRest != null) {
                logger->info("Found previous indices for the same vertex %s", index, previousSameVertexDiffRest->toString().c_str());
            ...
            logger->info("Updating geometry index %u", index);
...
private:
    void printLogInfo(GeometryResource *geometry) {
        ...
        for(std::vector<unsigned int>::iterator indexIterator = geometry->getIndices().begin(); indexIterator != geometry->getIndices().end(); indexIterator ++) {
            logger->info("%d: %s - %s - %s",
                    *indexIterator,
                    geometry->getVertices().at(*indexIterator).toString("%.6f").c_str(),
