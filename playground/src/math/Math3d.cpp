...
const real vector2::operator ^(const vector2 &op1) const {
	return(this->perpDotProduct(op1));
}
...
void vector2::normalizar() {
	...
	if(modulo != 0.0f) *this = *this / modulo;  // Tendría que ir una excepción si modulo = 0
}
const vector2 vector2::normalizado() const {
	real modulo = this->modulo();
	if(modulo != 0.0f) return(*this / modulo);
	return(*this); // Tendría que ir una excepción
}
...
//  \ \  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	const real vector3::ProductoEscalar(const vector3 &op2) const {
	...
	const vector3 vector3::ProductoVectorial(const vector3 &op2) const {
	...
	/*char *vector3::Print(char *adonde) const {
		wsprintf(adonde, "%d %d %d\n\0", (int)x, (int)y, (int)z);
		return(adonde);
	}
	*/
	...
	const vector3 vector3::Normalizado(void) const { // devuelve un vector3 con la misma dirección que aquel sobre el cual se aplica la operación, pero con módulo = 1
		real modulo = this->Modulo();

		if(modulo != 0.0f) 
			return(vector3(this->x / modulo, this->y / modulo, this->z / modulo));
		else return(*this);
	}
...
matriz_2x2::matriz_2x2(real _00, real _01, real _10, real _11) {
	this->_00 = _00; this->_01 = _01;
	this->_10 = _10; this->_11 = _11;
}
matriz_2x2::matriz_2x2(real angulo) {
	this->hacerRotacion(angulo);
}
matriz_2x2::matriz_2x2(const matriz_2x2 &op1) {
	*this = op1;
}
...
//  \ \  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	matriz_3x3::matriz_3x3(const matriz_3x3 &op1) { // Constructor de Copia
		memcpy(this->m, op1.m, sizeof(this->m));
	}
	matriz_3x3::matriz_3x3(void ) { // matriz Identidad
	...
	matriz_3x3::matriz_3x3(const matriz_4x4 &op1) {		// si no lo dejo, no funciona el operador (matriz_4x4)matriz_3x3. Da error de compilación por ambigüedad en la línea que llama a la función.
...
/*	char * matriz_3x3::Print(char *adonde) const {
		wsprintf(adonde, "%d	%d	%d\n%d	%d	%d\n%d	%d	%d",
...
*/
	...
	const vector3 matriz_3x3::Fila(int nro) const {
		switch(nro) {
			case 0:
				return(vector3(_00, _01, _02));
			break;
			case 1:
				return(vector3(_10, _11, _12));
			break;
			default:
				return(vector3(_20, _21, _22));
			break;
		}
	}
	const vector3 matriz_3x3::Columna(int nro) const {
		switch(nro) {
			case 0:
				return(vector3(_00, _10, _20));
			break;
			case 1:
				return(vector3(_01, _11, _21));
			break;
			default:
				return(vector3(_02, _12, _22));
			break;
		}
	}
	const vector3 matriz_3x3::operator() (int nro) const {
		return(this->Fila(nro));
	}

	real &matriz_3x3::operator()(int fila, int columna) {
		if(0 <= fila && fila <= 2 && 0 <= columna && columna <= 2) {
			return(this->m[fila * 2 + columna]);
		}
	}
...
//  \ \  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	matriz_4x4::matriz_4x4(const matriz_4x4 &op1) { //Constructor de copia
		memcpy(this->m, op1.m, sizeof(this->m));
	}
	...
	/*vector3 matriz_4x4::operator*(const vector3 &op1) const {
		return(vector3(_00 * op1.x + _10 * op1.y + _20 * op1.z + _30,
					_01 * op1.x + _11 * op1.y + _21 * op1.z + _31,
					_02 * op1.x + _12 * op1.y + _22 * op1.z + _32
		));
	}
	*/
	...
	/*operator D3DXMATRIX () {
		return(D3DXMATRIX(	_00, _01, _02, _03,
					_10, _11, _12, _13,
					_20, _21, _22, _23,
					_30, _31, _32, _33));
	}
...
/*	char * matriz_4x4::Print(char *adonde) const {
		wsprintf(adonde, "%d	%d	%d	%d\n%d	%d	%d	%d\n%d	%d	%d	%d\n%d	%d	%d	%d\n\0",
			(int)_00, (int)_01, (int)_02, (int)_03,
			(int)_10, (int)_11, (int)_12, (int)_13,
			(int)_20, (int)_21, (int)_22, (int)_23,
			(int)_30, (int)_31, (int)_32, (int)_33);
		return(adonde);
	}
*/	
	const cuaternion matriz_4x4::Fila(int nro) const {
		switch(nro) {
			case 0:
				return(cuaternion(_00, _01, _02, _03));
			break;
			case 1:
				return(cuaternion(_10, _11, _12, _13));
			break;
			case 2:
				return(cuaternion(_20, _21, _22, _23));
			break;
			default:
				return(cuaternion(_30, _31, _32, _33));
			break;
		}
	}

	const cuaternion matriz_4x4::Columna(int nro) const {
		switch(nro) {
			case 0:
				return(cuaternion(_00, _10, _20, _30));
			break;
			case 1:
				return(cuaternion(_01, _11, _21, _31));
			break;
			case 2:
				return(cuaternion(_02, _12, _22, _32));
			break;
			default:
				return(cuaternion(_03, _13, _23, _33));
			break;
		}
	}

	const cuaternion matriz_4x4::operator()(int nro) const {	// Si le cambio el parámetro a unsigned char, da error de ambigüedad al quere utilizar la función (No al querer compilar sin usarla en el programa principal, ya que el error de ambigüedad se da en la línea que llama, no en la función).
		return(this->Fila(nro));
	}

	real & matriz_4x4::operator ()(int fila, int columna) {
		if(0 <= fila && fila <= 3 && 0 <= columna && columna <= 3) {
			return(this->m[4 * columna + fila]); // se multiplica por columna y no por fila porque la matriz está definida así:    0  4  8  12
		}																													// 1  5  9  13
																															// 2  6  10 14 
	}																														// 3  7 11  15	
