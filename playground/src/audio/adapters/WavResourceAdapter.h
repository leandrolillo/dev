	...
	private:
		...
		virtual Resource *load(FileParser &fileParser, const String &mimeType) {
			...
			while(fileParser.read(&chunkHeader, sizeof(ChunkHeader), 1) == 1) {
				if(asString(chunkHeader.id) == "RIFF") {
					...
					if(sizeof(FormatChunk) >= chunkHeader.length) {
						...
						if(fileParser.read(formatChunk, sizeof(char), chunkHeader.length) != chunkHeader.length) {
							logger->error("Could not read fmt chunk");
							delete formatChunk;
							formatChunk = null;
						}
					} else {
						logger->error("Unsupported format chunk size: expected %u, found %u", sizeof(FormatChunk), chunkHeader.length);
					}
					...
					if(fileParser.read(buffer, sizeof(char), chunkHeader.length) != chunkHeader.length) {
						logger->error("Could not read data chunk");
						delete dataChunk;
						dataChunk = null;
					}

					dataChunk->data.insert(dataChunk->data.end(), buffer, buffer+chunkHeader.length);
				} else { // skip chunk
					logger->verbose("Skipping unsupported chunk: [%s] of size [%u]", asString(chunkHeader.id).c_str(), chunkHeader.length);
			...
			if(riffChunk != null && asString(riffChunk->waveId) == "WAVE") {
				if(formatChunk != null) {
					if(dataChunk != null && !dataChunk->data.empty()) {
						logger->verbose("Loading [%s] file [%s] [%u]bps [%u] bytes]",
								...
								formatChunk->numberOfSamplesPerSec * formatChunk->numberOfChannels,
								dataChunk->data.size());

						audioResource = new AudioResource(0, "audio/wav");
												audioResource->setFormat(formatChunk->numberOfChannels == 1 ? formatChunk->numberBytesPerSample == 1 ? MONO8 : MONO16 : formatChunk->numberBytesPerSample == 1 ? STEREO8 : STEREO16);
				...
				logger->error("Missing or unsupported RIFF chunk: [%s]", riffChunk != null ? asString(riffChunk->waveId).c_str() : "N/A");
			}
